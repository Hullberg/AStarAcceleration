...
HWOffsetFix<size,offset,sign_mode>::HWOffsetFix(varint_u<size> raw_bits)
	: m_bits(raw_bits), charPrint(false), stringPrint(false)
	{ }

template <uint size, int offset, SignMode sign_mode> inline
HWOffsetFix<size,offset,sign_mode>::HWOffsetFix(double v) {
	HWFloat<11,53> float_dbl(v);
	HWOffsetFix<size,offset,sign_mode> trueFix = cast_float2fixed<size,offset,sign_mode,TONEAREVEN>(float_dbl);
	*this = trueFix;
}
...
// Print help //////////////////////////////////////////////////////////
template <uint size, int offset, SignMode sign_mode> inline
HWOffsetFix<size,offset,sign_mode> HWOffsetFix<size,offset,sign_mode>::clearFormatting() const {
	const_cast<HWOffsetFix<size,offset,sign_mode>*>(this)->charPrint = false;
	const_cast<HWOffsetFix<size,offset,sign_mode>*>(this)->stringPrint = false;
	return *this;
}

template <uint size, int offset, SignMode sign_mode> inline
HWOffsetFix<size,offset,sign_mode> HWOffsetFix<size,offset,sign_mode>::printAsChar() const {
	const_cast<HWOffsetFix<size,offset,sign_mode>*>(this)->charPrint = true;
	return *this;
}

template <uint size, int offset, SignMode sign_mode> inline
HWOffsetFix<size,offset,sign_mode> HWOffsetFix<size,offset,sign_mode>::printAsString() const {
	const_cast<HWOffsetFix<size,offset,sign_mode>*>(this)->stringPrint = true;
	return *this;
}
...
_cast_fixed2fixed(const HWOffsetFix<a_size, a_offset, a_sign_mode> &a, EXCEPTOVERFLOW, Int2Type<true>) {
	...
	// Case (c) Output LSB further left
		...
		if (out_lsb <= in_msb + 1) {
			...
			// Attach a guard MSB (sign extend)
			copy_bits<0, 0, a_size>(out, in);
			pad_msb<a_size, a_sign_mode, a_sign_mode, 1>(out);

			// Cut off LSBs if necessary
			const int cutoff_offset = MAX(out_lsb - in_lsb, 1);
			const int cutoff_width = MAX((int) a_size + 1 - cutoff_offset, 0);
			...
			switch (round_mode) {
				case TRUNCATE:
					...
					break;
				case TONEAR:
					round_up = out[cutoff_offset - 1];
					break;
				case TONEAREVEN:
		            if (cutoff_offset == 1)
		            	round_up = out[1] && out[0];
		            else if (cutoff_offset == 2)
		            	round_up = out[1] && (out[0] || out[2]);
		            else {
		            	bool drop_lower = out.template slice<0, cutoff_offset - 1>();
		            	bool drop_msb = out[cutoff_offset - 1];
		            	bool keep_lsb = out[cutoff_offset];
			...
			if (cutoff_width > 0)
				out = out.template slice<cutoff_offset, cutoff_width>();

			// Round
			if (round_up)
				out++;
			for(int i=0; i<r_size;i++) {
				if(i >= cutoff_width) out[i] = 0;
			}
	...
	//  o:     |====
		...
		const int cutoff_width = MAX(in_msb_adjusted - out_msb, 1);
			 ...
			 (a_sign_mode == TWOSCOMPLEMENT && r_sign_mode == UNSIGNED))
		{
			...
			if (hw_exception) {
				varint_u<cutoff_width> cutoff = out.template slice<cutoff_index, cutoff_width>();
		...
		// Case (b2) UNSIGNED -> TWOSCOMPLEMENT
			...
			if (hw_exception) {
				varint_u<cutoff_width + 1> cutoff = out.template slice<cutoff_index - 1, cutoff_width + 1>();
...
DIV_fixed(,HWEXCEPT,EXCEPTFLOAT) {
	...
	if (r_sgn) {
		...
		result = result_extended.template slice<0, r_size>();
	}
