...
struct max_msi_handle {
	...
	std::unique_ptr<ManagerDesignRunnerSync> m_runner;
...
public:
	...
	max_msi_handle(const std::string &simulated_system_name, int device_index, int err_fd) :
		m_max_file(simulated_system_name, err_fd)
	{
	...
	void enableWatchnodes(const std::string &dir_name) {
		struct stat sb;
		if(stat(dir_name.c_str(), &sb) == -1 && errno == ENOENT) {
		...
		for (WatchSync *watch : watch_blocks)
...
int max_msi_initialize(max_msi_handle_t *const handle, const char *simulated_system_name, int device_index, int err_fd) try
{
	TRACE_FUNC_ENTRY
	...
	*handle = new max_msi_handle(simulated_system_name_str, device_index, err_fd);
...
{
	std::cerr << "max_msi_initialize: " << e.what();
	*handle = 0;
...
{
	std::cerr << "max_msi_initialize: unknown exception";
	*handle = 0;
	return EMAX_SIM_EXCEPTION_DURING_INIT;
}
