...
#ifdef NETWORKING_INCLUDED
#include "TCP.h"
#include "Ethernet.h"
#include "DualAspectFramed.h"
#endif /* NETWORKING_INCLUDED */
#include "LMemSim.h"
#include "Timestamp.h"
...
#ifdef DESIGN_NAME
	#warning "The macro DESIGN_NAME will be redefined in the maxfile"
#endif /* DESIGN_NAME */
#define DEFINE_DESIGN_NAME
	#include QUOTE(MAXFILE_INC)
#undef DEFINE_DESIGN_NAME
...
class MaxFile {
	...
	#define MANAGER_NODE_CPP_SIM_MODEL_CTOR(name, type, args...) \
		std::unique_ptr< type > MANAGER_BLOCK_FIELD(name);
	#include QUOTE(MAXFILE_INC)
	#undef MANAGER_NODE_CPP_SIM_MODEL_CTOR

	ManagerDesignSync m_design;
	std::unordered_map<std::string, ManagerBlock*> m_block_map;
	std::unordered_map<std::string, int> m_params_map;
	std::unordered_map<std::string, std::string> m_capabilities_map;

public:
	MaxFile(const std::string &simulated_system_name, int err_fd) :
		#define MANAGER_NODE_CPP_SIM_MODEL_CTOR(name, type, args...) \
		...
		m_design(err_fd)
	{
		...
		m_design.setDesignName( QUOTE(DESIGN_NAME) );
	...
	int getParamValue(std::string name) {
		std::unordered_map<std::string, int>::iterator v = m_params_map.find(name);
		if(v == m_params_map.end())
			throw SimException(bfmt("Param '%1%' not found in .max file.") % name);
	...
	int getCapability(std::string name, std::string &result) {
		std::unordered_map<std::string,std::string>::iterator iter = m_capabilities_map.find(name);
...
private:
		...
		const std::string &ip)
	{
		...
		if(src_class != dst_class) {
			...
			throw(SimException(bfmt("Unknown IO class.")));
		}
	}
