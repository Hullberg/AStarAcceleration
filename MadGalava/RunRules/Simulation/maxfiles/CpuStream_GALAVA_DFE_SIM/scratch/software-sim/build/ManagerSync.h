...
private:
	...
	std::vector<ManagerBlockIrqSync *> m_interrupt_source; // list of all IRQ sources (if any)

	std::string m_design_name;

	std::shared_ptr<std::ostream> m_default_debug_output_stream;
	std::shared_ptr<std::ostream> m_default_debug_output_backup;
	std::shared_ptr<std::ostream> m_error_stream;
...
public:
	...
	std::ostream& getErrorStream() const         {return *m_error_stream;}
	bool useDebugBackup() const {return m_use_debug_backup;}


	// MODIFIERS
	explicit ManagerDesignSync(int err_fd);
	...
	std::vector<T*> getBlocksOfType() {
		...
		for (ManagerBlockSync *block : m_blocks) {
...
private:
	...
	mutable std::mutex m_start_stop_mutex;
	CondVar m_abort_cond_var;
